...
class FrehageSolverWithPartialStates(_sul: SUL) extends BaseSolver {
  ...
  while (moduleQueue.values.exists(q => q.nonEmpty)) {
    ...
    for (m <- model.modules) {
      ...
      moduleQueue += (m -> moduleQueue(m).filter(s =>
        !moduleStates(m).contains(getReducedStateMap(s, model, m))
      ))

      moduleStates += (m -> (moduleStates(m) ++ moduleQueue(m).map(s =>
        getReducedStateMap(s, model, m)
      )))

      val next =
        moduleQueue(m).flatMap(s => _sul.getOutgoingTransitions(s, model.eventMapping(m)))
      moduleTransitions += (m -> (moduleTransitions(m) ++ next.map(t =>
        getReducedStateMapTransition(t, model, m)
      )))
  ...
  override def getAutomata: Automata = {
    ...
    val modules = for {
      ...
      states: Map[StateMap, State] = moduleStates(m)
        ...
        .toMap
      transitions: Set[Transition] = moduleTransitions(m).map(t =>
        Transition(states(t.source), states(t.target), t.event)
      )
