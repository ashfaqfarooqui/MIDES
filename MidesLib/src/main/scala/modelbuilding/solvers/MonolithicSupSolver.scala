...
class MonolithicSupSolver(_sul: SUL) extends BaseSolver with Logging {
  ...
  val specs =
    _sul.specification.get //SupremicaWatersSystem(_model.specFilePath.get).getSupremicaSpecs.asScala.toSet
  ...
  def mapStates(states: Set[StateMap]): Map[StateMap, State] = {
    states.zipWithIndex.toMap.map { case (sm, i) =>
      (sm, if (sm.equals(initState)) State("init") else State(s"s$i"))
    }
  }
  ...
  val markedStateSet =
    allStatesAsStateMap.filter(extendedGoalPredicate.eval(_).get) map mappedStates
