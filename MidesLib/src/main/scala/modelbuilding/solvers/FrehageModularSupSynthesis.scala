...
class FrehageModularSupSynthesis(_sul: SUL) extends BaseSolver {
  ...
  def getSupervisorModules(
    ...
    if (!model.isModular) Set(Module(model.name, model.states, model.alphabet))
      ...
      if (
        supremicaSpecs.isEmpty
      ) // If no Automata specs exist the original modules can be used.
        modularModel.modules.map(m =>
          Module(m, modularModel.stateMapping(m), modularModel.eventMapping(m))
        )
      else {

        val specAlphabets: Map[String, Alphabet] = supremicaSpecs.map(s =>
          s._1 -> new Alphabet(
        ...
        val moduleEventMapping: Map[String, Alphabet] = specAlphabets.map(s =>
          s._1
...
//        val moduleStateMapping: Map[String, StateSet] = moduleEventMapping.mapValues{ a1 =>
  ...
  private val remainingPlantModules: Set[String] = model.modules.filterNot(m =>
    specModules.exists(s => model.eventMapping(m).events.subsetOf(s.alphabet.events))
  )
  private val plantModules: Set[Module] = remainingPlantModules.map(m =>
    Module(m, model.stateMapping(m), model.eventMapping(m))
  )
      ...
      for (m <- concernedModules) {
        if (plantModules.contains(m) || specTargetStates(m.name).nonEmpty) {
          ...
          if (!moduleStates(m.name).contains(tReduced.target)) {
            ...
            moduleForbidden(m.name).contains(tReduced.target) && !t.event.isControllable
          ) {
  ...
  for (m <- modules) {
    ...
    val nonLocalVariables =
      moduleTransitions(m.name)
        ...
        .flatMap(t =>
          t.source.states.keys.filter(k => t.source.states(k) != t.target.states(k))
        )
    ...
    reducedModuleTransitions(m.name) --= reducedModuleTransitions(m.name).filter(t =>
  ...
  override def getAutomata: Automata = {
    val automatons = for {
      ...
      states: Map[StateMap, State] = reducedModuleStates(m.name)
        .map(s => {
          val name = ((if (
                         s.states.forall { case (k, v) => initState.states(k) == v }
