...
class SupremicaWatersSystem(
    val mModule: ModuleSubject =
      SimpleModuleFactory("NewModule") /* e.g. ReadSystemFromWmodFile("supremicaFiles/controlabilitytest.xml") */)
...
trait ParseTextFileToModule
  ...
  def parseTextFileToModule(iTextFilePath: String): Boolean = {
    ...
    lazy val variables = lines.flatMap(str =>
      parseAll(
...
//    fscs.foreach(d => addForbiddenExpression(valuesFromStringToIntRepresentation(d), addSelfLoop = false, addInComment = true))
    fscs.foreach(d =>
      addForbiddenExpression(valuesFromStringToIntRepresentation(d), true, false)
    )
  ...
  private def valuesFromStringToIntRepresentationFactory(
    ...
    valueDomain
      ...
      .filter(v =>
        !mValueStringIntMap(v).equals(Integer.toString(valueDomain.indexOf(v)))
      )
      .foreach { v =>
        println(
    ...
    mValueStringIntMap = valueDomain.foldLeft(mValueStringIntMap) { case (acc, v) =>
      acc + (v -> Integer.toString(valueDomain.indexOf(v)))
    }
    return true
  }
  private def valuesFromStringToIntRepresentation(expr: String): String =
    mValueStringIntMap.foldLeft(expr) { case (acc, (k, v)) =>
      acc.replaceAll(s"\\b$k\\b", v)
    }
