...
trait BasePopulater extends Workers {
  ...
  def addVariable(
    ...
    markedValues.foreach(value =>
      if (value < lowerBound || value > upperBound) {
        println(
          "Problem when creating variable, a marked value not in domain:\n Variable: " + name
        ); return None
      }
    )
    ...
    addVariable(
      name,
      range,
      mFactory.createIntConstantProxy(initialValue),
      markedValues.map(Integer.toString(_))
    )
  }
  ...
  def addFlowerRoot(name: String): Option[SimpleComponentSubject] = {
    if (getEFAs.map(_.getName()).contains(name)) {
      return None
    }
  ...
  private def addLocation(
    ...
    if (graph.getNodesModifiable().asScala.map(_.getName()).contains(name)) {
      return None
    }
    val propList = Seq(
      mFactory.createSimpleIdentifierProxy(EventDeclProxy.DEFAULT_MARKING_NAME)
    )
    val node = mFactory.createSimpleNodeProxy(
      name,
      mFactory.createPlainEventListProxy(propList.asJava),
      null,
      isInitial,
      null,
      null,
  ...
  def addEventIfNeededElseReturnExistingEvent(
      eventName: String,
      unControllable: Boolean = false
    ): Some[EventDeclProxy] = {
    ...
    if (!result.isEmpty) Some(result.head)
      ...
      val edp = mFactory.createEventDeclProxy(
        ident,
        if (unControllable) EventKind.UNCONTROLLABLE else EventKind.CONTROLLABLE
      )
  ...
  private def addEdge(
      ...
      fromNode: NodeProxy,
      toNode: NodeProxy,
      guard: Option[SimpleExpressionSubject],
      actions: Option[Set[BinaryExpressionSubject]]
    ): Option[EdgeSubject] = {
    val labelBlock = mFactory.createLabelBlockProxy(
      Seq(mFactory.createSimpleIdentifierProxy(eventName)).asJava,
      null
    );
    val guardActionBlock = mFactory.createGuardActionBlockProxy()
    if (guard.isDefined) {
      guardActionBlock.getGuardsModifiable().add(guard.get)
    }
    if (actions.isDefined) {
      actions.get.foreach(guardActionBlock.getActionsModifiable().add(_))
    }
    val edge = mFactory.createEdgeProxy(
      fromNode,
      toNode,
      labelBlock,
      guardActionBlock,
