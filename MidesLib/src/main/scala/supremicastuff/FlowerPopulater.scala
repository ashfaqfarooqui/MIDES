...
trait FlowerPopulater extends BasePopulater with Algorithms {
  ...
  def forbiddenEventNameFactory(): String = {
    val suggestion =
      NamingInAutogeneratedFlowers.FORBIDDEN_EVENT_NAME + indexCounterFactory
    if (getAlphabet.map(_.getName()).contains(suggestion)) forbiddenEventNameFactory
    else suggestion
  }

  def forbiddenVariableNameFactory(): String = {
    val suggestion =
      NamingInAutogeneratedFlowers.FORBIDDEN_VARIABLE_NAME + indexCounterFactory
    if (getVariables.map(_.getName()).contains(suggestion)) forbiddenVariableNameFactory
    else suggestion
  }
  ...
  def getFreshFlower(eventLabel: String): Option[SimpleComponentSubject] = {
    getFlowers.foreach(efa =>
      if (!efaContainsEventLabel(efa, eventLabel)) {
        return Some(efa)
      }
    )
    ...
    efa
      ...
      .foreach(
        _.getLabelBlock()
          ...
          .asScala
          .foreach(event =>
            if (event.toString().equals(eventLabel)) {
              return true
            }
          )
      )
  ...
  def addLeaf(
    ...
    getFreshFlower(eventName).flatMap(
      addLeafToEfa(_, eventName, guardAsText, actionsAsText)
    )
  }

  def addLeafAndEventToAlphabet(
      eventName: String,
  ...
  /*
  def addSupervisorGuardsToFreshFlower(
      optSupervisorEventGuardMap: Option[Map[String, String]] =
        SupervisorAsBDD().getSupervisorGuards
    ): Option[SimpleComponentSubject] = {
    val eventGuardMap = optSupervisorEventGuardMap.map(_.filter {
      case (_, guard) => !guard.equals("1")
    })
    ...
    addFlowerRoot(efaNameFactory).map(efa => {
      eventGuardMap.map(_.foreach {
        case (label, guard) =>
          if (addLeafToEfa(efa, label, guard, null).isEmpty) {
            return None
          }
      });
      efa
  ...
  def addForbiddenExpression(
      forbiddenExpression: String,
      addSelfLoop: Boolean = true,
      addInComment: Boolean = false
    ): Unit = {
    ...
    if (addSelfLoop) {
      ...
      addLeafAndEventToAlphabet(
        forbiddenEventNameFactory(),
        true,
        s"($forbiddenExpression)&$mForbiddenVariableName==0",
        s"$mForbiddenVariableName=1"
      )
    }
    if (addInComment) {
      mModule.setComment(
        s"$getComment${TextFilePrefix.FORBIDDEN_PREFIX}$forbiddenExpression"
      )
    }
  }
}
