...
trait Workers extends Algorithms {
  ...
  def parseGuardStr[T](guardAsText: String): Option[T] = {
    try {
      ...
      case _: Throwable => println("Problem when parsing guard:\n Guard: " + guardAsText)
    }
    None
  }
  ...
  def evalExpression(
      exp: SimpleExpressionProxy
    )(state: Map[String, Int]
    ): Option[Boolean] = {
    // Translate state into a bindingcontext
    val mbc = new ModuleBindingContext(mModule)

    state.foreach { case (variable, value) =>
      try {
        mbc.insertBinding(
          mFactory.createSimpleIdentifierProxy(variable),
          mFactory.createIntConstantProxy(value)
        )
      } catch {
        case _: Throwable =>
          println("Problem when inserting binding to variable:\n Variable: " + variable);
          return None
      }
    }
    ...
    try {
      ...
      case _: Throwable =>
        println("Problem when evaluating expression:\n Expression: " + exp); return None
    }
  }

  // some Semantic check
  def semanticsOK: Boolean = if (getDFA.isEmpty) {
    println("Problem with the semantics! Probably an undefined value for some variable.");
    false
  } else true
}
