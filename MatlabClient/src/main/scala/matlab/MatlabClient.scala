...
class MatlabClient extends Logging {
  ...
  def MatlabClient: MatlabEngine = {

    getEngine
  }
  def getEngine: MatlabEngine = {
    if (mEngine.isDefined) mEngine.get
    else startMatlab.get
  }

  def startMatlab: Option[MatlabEngine] = {
  ...
  def closeMatlab(): Unit = getEngine.disconnect()

  def runFunction(retArgs: Int, functionName: String, param: java.lang.Object): Object = {
...
object MatlabClient {
  ...
  def destroy(): Unit       = MatlabClient().closeMatlab()
}
