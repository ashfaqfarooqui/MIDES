package modelbuilding.algorithms.CLStar
...
object CObservationTable {
  ...
  def apply(
      ...
      instance: Int
    ): CObservationTable = new CObservationTable(table, A, S, E, isMember, instance)
  def apply(
      A: Alphabet,
      S: Set[Grammar],
      E: Set[Grammar],
      isMember: Grammar => Int,
      instance: Int
    ): CObservationTable = new CObservationTable(initTable, A, S, E, isMember, instance)

  def updateTable(
      oldTable: CObservationTable,
      newS: Set[Grammar],
      newE: Set[Grammar]
    ): CObservationTable = {
