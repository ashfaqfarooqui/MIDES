...
class SimulateAgv extends Simulator {
  ...
  override val guards: Map[Command,Predicate] = Map(
    ...
    e10 -> AND(EQ(v1p, "i1"), EQ(in1, true)),
    e11 -> EQ(in1, false),
    e20 -> AND(EQ(v5p, "o"), EQ(out, true)),
    e21 -> EQ(out, false),
    e30 -> AND(EQ(v2p, "i3"), EQ(in3, true)),
    e31 -> EQ(in3, false),
    ...
    w12 -> EQ(w1_2, true),
    w13 -> AND(EQ(v5p, "w1o"), EQ(w1_3, true)),
    w14 -> AND(EQ(v3p, "w1i1"), EQ(w1_4, true)),
    w15 -> AND(EQ(v4p, "w1i2"), EQ(w1_5, true)),
    w21 -> EQ(w2, "w21"),
    w22 -> AND(EQ(v3p, "w2o"), EQ(w2, "w22")),
    w23 -> AND(EQ(v1p, "w2i"), EQ(w2, "w23")),
    w24 -> EQ(w2, "w24"),
    w31 -> AND(EQ(v4p, "w3o"), EQ(w3, "w31")),
    w32 -> AND(EQ(v2p, "w3i"), EQ(w3, "w32")),
    w33 -> EQ(w3, "w33"),
    w34 -> EQ(w3, "w34"),
  )
  ...
  override val actions: Map[Command,List[Action]] = Map(
    ...
    e11 -> List(Toggle(in1)),
    e20 -> List(Assign(v5l, false), Toggle(out)),
    e21 -> List(Toggle(out)),
