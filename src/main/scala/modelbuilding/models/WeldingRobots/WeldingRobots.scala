...
class WeldingRobots(n_robots: Int, n_tasks: Int) extends ModularModel {
  ...
  override val name: String = "WeldingRobots"
  ...
  val stateString: String = "s " + (1 to n_robots).map(r => s"r_${r}_l r_${r}_t").mkString(" ")
  override val states: StateSet = StateSet(stateString.split(" ").toSet)

  override def stateMapping: Map[String, StateSet] =
    (1 to n_robots).map(r => s"R$r" -> StateSet((1 to n_robots).flatMap(r => s"r_${r}_l r_${r}_t".split(" ")).toSet + "s")).toMap
  ...
  stateMapping.foreach(println)
  eventMapping.foreach(println)
