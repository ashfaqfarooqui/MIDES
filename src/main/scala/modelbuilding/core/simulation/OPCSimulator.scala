...
trait OPCSimulator extends Simulator with OPCVariables with Logging{
  ...
  def subscribeToAllVars={
    getClient.subscribeToNodes(variableList.get)
  }
  ...
  def isCommandDone = getClient.getState.getOrElse(stateExecVariable,"None").toString==stateExecFinishedValue
    ...
    evalCommandToRun(c, s, acceptPartialStates) match {
      case Some(true) => c match {
        ...
        case _ =>
          ...
          while(!isCommandDone && deadline.hasTimeLeft()){
            wait(1)
