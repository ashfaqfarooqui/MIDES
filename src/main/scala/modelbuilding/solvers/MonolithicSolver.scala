...
class MonolithicSolver(model:Model) extends BaseSolver {
  ...
  def explore(queue:Queue[StateMap],visitedSet:Set[StateMap], arcs:Set[StateMapTransition]):Set[StateMapTransition]={
    if(queue.isEmpty) {
      ...
      val reachedStates = events.map(e => sul.getNextState(currState._1, e.getCommand) match {
        case Some(value) => transitions = transitions + StateMapTransition(currState._1,value, e)
